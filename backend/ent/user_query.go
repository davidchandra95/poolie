...
func (_q *UserQuery) QueryRides() *RideQuery {
...
func (_q *UserQuery) QueryVehicles() *VehicleQuery {
...
func (_q *UserQuery) WithRides(opts ...func(*RideQuery)) *UserQuery {
	query := (&RideClient{config: _q.config}).Query()
...
func (_q *UserQuery) WithVehicles(opts ...func(*VehicleQuery)) *UserQuery {
	query := (&VehicleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withRides; query != nil {
		if err := _q.loadRides(ctx, query, nodes,
			func(n *User) { n.Edges.Rides = []*Ride{} },
			func(n *User, e *Ride) { n.Edges.Rides = append(n.Edges.Rides, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withVehicles; query != nil {
		if err := _q.loadVehicles(ctx, query, nodes,
			func(n *User) { n.Edges.Vehicles = []*Vehicle{} },
			func(n *User, e *Vehicle) { n.Edges.Vehicles = append(n.Edges.Vehicles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadRides(ctx context.Context, query *RideQuery, nodes []*User, init func(*User), assign func(*User, *Ride)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(ride.FieldDriverID)
	}
	query.Where(predicate.Ride(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.RidesColumn), fks...))
	}))
...
func (_q *UserQuery) loadBookings(ctx context.Context, query *BookingQuery, nodes []*User, init func(*User), assign func(*User, *Booking)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(booking.FieldPassengerID)
	}
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "passenger_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *UserQuery) loadVehicles(ctx context.Context, query *VehicleQuery, nodes []*User, init func(*User), assign func(*User, *Vehicle)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(vehicle.FieldUserID)
	}
	query.Where(predicate.Vehicle(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.VehiclesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
